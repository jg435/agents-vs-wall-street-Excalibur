...
<p><b>How load-bearing is it?</b> The 0.8 weight applies to 7 of our 12
numbers, and on six of those the consensus-to-guide gap is so small that any
weight between 0 and 1 would move the forecast by less than 2.5% — on Home
Depot's net sales, by 0.09%. It is materially load-bearing on exactly one
number, Hays operating profit, where it accounts for a 4.6% move off
consensus.</p>
<p><b>Post-hoc audit of our own constant:</b> late in the day we estimated the
weight properly on 29 quarters of ADI history. It does not survive — the
fitted weight is negative, 0.8 sits outside its confidence interval, and out
of sample the rule loses to leaving consensus alone; what actually predicts
ADI is a persistent ~5.6% beat over consensus (22/24 quarters) that neither
consensus nor guidance captures. A revised ADI forecast built on that measured
beat prior was prepared and preserved on a branch
(<code>viktorideas</code>, with the study in the repo); the team shipped the
uniform-rule values as submitted. We publish the negative finding rather than
hide it: the check we built to audit ourselves is the thing that found it.</p>
